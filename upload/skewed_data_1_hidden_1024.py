...
param = {
    ...
    "M_train": 100,           # training MMD 阶段每个 Z 生成的样本数，可调
    "k_value": 8,
    "j_value": 1000,
    "noise_dimension": 5,
    "noise_dimension_type": "normal",
    "noise_dimension_var": 1,
    "hidden_layer_size": 1024,   # 1 hidden layer, width = 1024
    "normal_ini": False,
    "preprocess": 'scale',
    "G_lr": 5e-5,
    "alpha": 0.1,
...
class Generator(torch.nn.Module):
    """
    One-hidden-layer conditional generator.

    Architecture:
        concat(Z, noise) -> Linear(hidden_layer_size) -> LeakyReLU
        -> Dropout -> Linear(output_dimension)

    With the current parameter setting, hidden_layer_size = 1024.
    """
    ...
    def __init__(self, input_dimension, output_dimension, noise_dimension, hidden_layer_size,
        ...
        self.fc1 = torch.nn.Linear(
            input_dimension + noise_dimension,
            hidden_layer_size,
            bias=True
        )
...
def mGAN(n=500, z_dim=1, simulation='type1error', batch_size=64, epochs_num=1000,
    ...
    # ============================================================
    # Correct median heuristic for kernel bandwidths
    # Use pairwise distances, not kernel values.
    #
    # Original problematic code:
    #   u_mx = torch.exp(-torch.abs(y.repeat(1, n) - y.repeat(1, n).T))
    #   sigma_u = torch.median(u_mx).item()
    #
    # That uses the median of kernel values exp(-distance), not the
    # median of distances. Here we use the median of pairwise L1 distances.
    # ============================================================

    # Bandwidth for Z kernel: median of ||Zi - Zj||_1
    w_dist = torch.linalg.vector_norm(
        z.repeat(n, 1, 1) - torch.swapaxes(z.repeat(n, 1, 1), 0, 1),
        ord=1,
        dim=2
    )
    w_dist_nonzero = w_dist[w_dist > 0]
    if w_dist_nonzero.numel() == 0:
        sigma_w = 1.0
    else:
        sigma_w = torch.median(w_dist_nonzero).clamp_min(1e-8).item()

    # Bandwidth for Y kernel: median of ||Yi - Yj||_1
    y_dist = torch.linalg.vector_norm(
        y.repeat(n, 1, 1) - torch.swapaxes(y.repeat(n, 1, 1), 0, 1),
        ord=1,
        dim=2
    )
    y_dist_nonzero = y_dist[y_dist > 0]
    if y_dist_nonzero.numel() == 0:
        sigma_u = 1.0
    else:
        sigma_u = torch.median(y_dist_nonzero).clamp_min(1e-8).item()

    # Bandwidth for X kernel: median of ||Xi - Xj||_1
    x_dist = torch.linalg.vector_norm(
        x.repeat(n, 1, 1) - torch.swapaxes(x.repeat(n, 1, 1), 0, 1),
        ord=1,
        dim=2
    )
    x_dist_nonzero = x_dist[x_dist > 0]
    if x_dist_nonzero.numel() == 0:
        sigma_v = 1.0
    else:
        sigma_v = torch.median(x_dist_nonzero).clamp_min(1e-8).item()
    ...
    if not using_orcale:
        G_zy = Generator(
            z_dim, y_dims, noise_dimension, hidden_layer_size,
            False, 0.1, drop_out_p
        ).to(device)

        G_zx = Generator(
            z_dim, x_dims, noise_dimension, hidden_layer_size,
            False, 0.1, drop_out_p
        ).to(device)
    ...
    for k_fold in range(k):
        ...
        for i, (x_test, y_test, z_test) in enumerate(dataloader_test):
            ...
            if not using_orcale:
                ...
                with torch.no_grad():
                    fake_x = G_zx(
                        torch.cat((z_test_temp, Noise_fake), dim=1)
                    ).reshape(1, -1)
    ...
    return np.mean(
        torch.mean(boot_temp_all, dim=0).numpy() > torch.mean(stat_all).item()
    )
# ============================================================
# 并行部分
# ============================================================
